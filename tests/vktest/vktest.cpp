...
private:
    ...
    glm::vec3 direction = glm::normalize(glm::vec3(0.0f, 0.0f, 0.0f) - cameraPosition);

    float yaw = glm::degrees(atan2(direction.z, direction.x)); // Yaw is the angle in the XZ plane
    float pitch = glm::degrees(asin(direction.y));  

    glm::vec3 cameraFront = glm::vec3(0.0f, -1.0f, 0.0f);

    float deltaTime = 0.02f; // Time between current frame and last frame
    float lastFrame = 0.02f; // Time of the last frame
    ...
    void processInput() {
        ...
        const float cameraSpeed = 2.5f * deltaTime; // Adjust speed based on delta time
    
        if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
            moveCamera(glm::vec3(0.0f, 0.0f, cameraSpeed)); // Move forward
        }
        if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
            moveCamera(glm::vec3(0.0f, 0.0f, -cameraSpeed)); // Move backward
        }
        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
            moveCamera(glm::vec3(-cameraSpeed, 0.0f, 0.0f)); // Move left
        }
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
            moveCamera(glm::vec3(cameraSpeed, 0.0f, 0.0f)); // Move right
        }
        if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS) {
            moveCamera(glm::vec3(0.0f, -cameraSpeed, 0.0f)); // Move down
        }
        if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) {
            moveCamera(glm::vec3(0.0f, cameraSpeed, 0.0f)); // Move up
        }
    }

    static void toggleMouseLock(GLFWwindow* window) {
        auto app = reinterpret_cast<HelloTriangleApplication*>(glfwGetWindowUserPointer(window));
        app->mouseLock = !app->mouseLock;
    ...
    static void mouseCallback(GLFWwindow* window, double xpos, double ypos) {
        ...
        float dy = app->lastY - ypos; // Reversed since y-coordinates go from bottom to top
    
        app->lastX = xpos;
        app->lastY = ypos;
    
        app->processMouseMovement(dx, dy);
    }
    ...
    void createImage(int width, int height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
        ...
        allocInfo.memoryTypeIndex = findMemoryType(
            ...
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT // Best for images
        );
    ...
    void createDepthResources() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            depthImageViews.push_back(createImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT));
            ...
            this->depthImageMemories.push_back(depthImageMemory);
        }
    }
    ...
    void createRenderPass() {
        ...
        depthAttachment.format = findDepthFormat(); // e.g., VK_FORMAT_D32_SFLOAT
...
int main() {
    ...
    MeshGen::generateMesh(vertices, indices, "/Users/jamesmaxwell/Documents/Projects/Terrain_CFD/Data/out_data.txt", 1.0f, 10.0f);
