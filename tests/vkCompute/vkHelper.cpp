...
{
    vk.enableValidationLayers = enable;
    if (enable) {
...
void createInstance(vulkan &vk, std::string appName, std::string engineName)
{
    ...
    auto extensions = getRequiredExtensions(vk.enableValidationLayers);
...
void createCommandPool(vulkan& vk) {
    QueueFamilyIndices queueFamilyIndices = findQueueFamilies(vk.physicalDevice, vk.surface);
...
void setupDebugMessenger(vulkan& vk) {
    if (!vk.enableValidationLayers) return;

    VkDebugUtilsMessengerCreateInfoEXT createInfo;
    populateDebugMessengerCreateInfo(createInfo);

    if (CreateDebugUtilsMessengerEXT(vk.instance, &createInfo, nullptr, &vk.debugMessenger) != VK_SUCCESS) {
